...
void __thiscall Redirected_Draw_Crosshair(void* Unknown_Parameter)
{
	*(__int8*)608051340 = 0;
	...
	auto Uber_Alles = [&](__int32 X, __int8 Red, __int32 Y)
	{
		...
		if (Red == 0)
		{
			(*Surface_Set_Color(*(unsigned __int32*)Surface + 44))(Surface, 0, 0, 0, 255);
		}
		else
		{
			(*Surface_Set_Color(*(unsigned __int32*)Surface + 44))(Surface, 255, 0, 0, 255);
		}
		...
		(*Surface_Draw_Line(*(unsigned __int32*)Surface + 60))(Surface, X + Points[0], Y - Points[1], X + Points[2], Y - Points[3]);

		(*Surface_Draw_Line(*(unsigned __int32*)Surface + 60))(Surface, X, Y, X + Points[4], Y - Points[5]);

		(*Surface_Draw_Line(*(unsigned __int32*)Surface + 60))(Surface, X + Points[4], Y - Points[5], X + Points[6], Y - Points[7]);

		(*Surface_Draw_Line(*(unsigned __int32*)Surface + 60))(Surface, X, Y, X + Points[8], Y - Points[9]);

		(*Surface_Draw_Line(*(unsigned __int32*)Surface + 60))(Surface, X + Points[8], Y - Points[9], X + Points[10], Y - Points[11]);

		(*Surface_Draw_Line(*(unsigned __int32*)Surface + 60))(Surface, X, Y, X + Points[12], Y - Points[13]);
